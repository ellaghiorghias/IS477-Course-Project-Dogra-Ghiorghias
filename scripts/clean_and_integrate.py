...
Cleans and integrates Zillow ZHVI and Census ACS datasets.

Steps:
  1. Load and reshape Zillow ZHVI from wide to long format
  2. Aggregate monthly ZHVI to annual medians per city
  3. Parse and normalize Census city names
  4. Build a city-name crosswalk via fuzzy matching
  5. Join datasets and compute price-to-income ratio
  6. Save cleaned and integrated outputs
"""
...
def parse_census_name(name):
    """Extract (city_clean, state_name) from a Census place name."""
    m = re.match(r"^(.+?)(?:\s+(?:city|town|village|borough|CDP|municipality|township))?\s*,\s*(.+)$",
                 name, re.IGNORECASE)
    if m:
...
def clean_zillow():
    ...
    df_long = df[meta_cols + date_cols].melt(
        id_vars=meta_cols, value_vars=date_cols, var_name="date", value_name="zhvi")
    ...
    annual = annual.rename(columns={"RegionName":"city","StateName":"state_name","State":"state_abbrev"})

    stats = annual.groupby(["state_abbrev","year"])["zhvi_annual_median"].agg(["mean","std"]).reset_index()
    annual = annual.merge(stats, on=["state_abbrev","year"], how="left")
    annual["zhvi_outlier_flag"] = ((annual["zhvi_annual_median"] - annual["mean"]).abs() > 4 * annual["std"])
...
def clean_census():
    ...
    null_frac = df.groupby(["state_fips","place_fips"])["median_household_income"].apply(lambda s: s.isna().mean())
    keep = null_frac[null_frac <= MAX_NULL_FRAC].index
    df = df[df.set_index(["state_fips","place_fips"]).index.isin(keep)].copy()

    result = df[["city_clean","state_name","state_fips","place_fips","year","median_household_income"]]
...
def build_crosswalk(zillow_df, census_df):
    ...
    zillow_state_names = (zillow_df[["state_abbrev","state_name"]].drop_duplicates()
                          .set_index("state_abbrev")["state_name"].to_dict())
    ...
    for _, zrow in zillow_df[["city","state_abbrev"]].drop_duplicates().iterrows():
        ...
        if not candidates:
            rows.append({"zillow_city":z_city,"zillow_state":z_state,"census_city_clean":None,
                         "census_state_name":z_state_name,"match_score":0,"manual_review":False})
            continue
        match = process.extractOne(z_city, candidates, scorer=fuzz.WRatio)
        rows.append({"zillow_city":z_city,"zillow_state":z_state,
    ...
    print(f"  {(~crosswalk.manual_review & crosswalk.census_city_clean.notna()).sum():,} auto-matched")
    print(f"  {crosswalk.manual_review.sum():,} flagged for manual review")
    return crosswalk
...
def integrate(zillow_df, census_df, crosswalk):
    ...
    merged = zillow_df.merge(
        crosswalk[["zillow_city","zillow_state","census_city_clean","census_state_name","match_score"]],
        left_on=["city","state_abbrev"], right_on=["zillow_city","zillow_state"], how="inner")
    ...
    final = merged.merge(
        ...
        right_on=["city_clean","state_name","year"], how="inner")

    final["price_to_income_ratio"] = final["zhvi_annual_median"] / final["median_household_income"]
    out = (final[["city","state_abbrev","year","zhvi_annual_median",
    ...
    print(f"  Saved {len(out):,} rows | {out[["city","state"]].drop_duplicates().shape[0]:,} cities")
    return out
