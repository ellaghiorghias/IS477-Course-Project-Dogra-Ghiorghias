...
def load_data():
    ...
    n_cities = df[["city", "state"]].drop_duplicates().shape[0]
    print(f"  {len(df):,} rows, {n_cities:,} cities")
    return df
...
def plot_top10_deterioration(df):
    ...
    pivot = df.pivot_table(index=["city", "state"], columns="year",
                           values="price_to_income_ratio")
...
def plot_income_vs_price_growth(df):
    ...
    piv_zhvi   = base.pivot_table(index=["city", "state"], columns="year",
                                   values="zhvi_annual_median")
    piv_income = base.pivot_table(index=["city", "state"], columns="year",
    ...
    ax.axline((0, 0), slope=1, color="red", linestyle="--",
              linewidth=1.2, label="Equal growth line")
...
def save_summary_stats(df):
    ...
    summary["pct_cities_ratio_above_5"] = (
        df.groupby("year")
        .apply(lambda g: (g["price_to_income_ratio"] > 5).mean() * 100)
        .values
    )
    summary["pct_cities_ratio_above_10"] = (
        df.groupby("year")
        .apply(lambda g: (g["price_to_income_ratio"] > 10).mean() * 100)
        .values
    )
